...
const CHAIN_META = {
  [arbitrumSepolia.id]: { label: "Arbitrum Sepolia", short: "Arb Sepolia", color: "text-emerald-400", dot: "bg-emerald-400", fhe: true  },
  [sepolia.id]:          { label: "Ethereum Sepolia", short: "Eth Sepolia",  color: "text-amber-400",   dot: "bg-amber-400",   fhe: false },
};

const getStoredUser  = () => { try { return JSON.parse(localStorage.getItem("cp_user") || "null"); } catch { return null; } };
const getStoredToken = () => localStorage.getItem("cp_token") || null;
const clearSession   = () => { localStorage.removeItem("cp_token"); localStorage.removeItem("cp_address"); localStorage.removeItem("cp_user"); };
...
const Navbar = ({ onLogout }) => {
  ...
  // Always navigate to /login — LoginFlow handles wallet connection internally
    ...
    return (
      <button onClick={() => handleSwitch(chain.id)} disabled={switching || active}
        className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-bold transition-all ${
          active
            ? meta.fhe ? "bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 cursor-default"
        ...
        <div className="flex items-center gap-2">
          {switching && !active
            ? <svg className="w-3 h-3 animate-spin text-slate-400" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>
            : <span className={`w-2 h-2 rounded-full flex-shrink-0 ${meta.dot}`}/>
          }
          <span>{meta.label}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className={`text-[9px] font-black px-1.5 py-0.5 rounded ${meta.fhe ? "bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400" : "bg-slate-100 dark:bg-slate-800 text-slate-400"}`}>
            {meta.fhe ? "FHE" : "No FHE"}
          </span>
          {active && <svg className="w-3.5 h-3.5 text-current" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M5 13l4 4L19 7"/></svg>}
        </div>
      </button>
    );
  };
  ...
  const LoginButton = ({ mobile = false }) => (
    <button onClick={handleLoginClick}
      className={`flex items-center gap-2 rounded-xl bg-indigo-600 hover:bg-indigo-500
        text-white font-black transition-all shadow-lg shadow-indigo-600/20 active:scale-95
        ${mobile ? "w-full justify-center px-4 py-3 text-sm" : "px-5 py-2 text-sm"}`}>
    ...
    <nav className="fixed top-0 left-0 w-full z-[100] transition-all duration-300
      ...
      {isWrongChain && (
        <div className="bg-amber-500 text-white py-2 px-4 flex items-center justify-center gap-3">
          <span className="text-[10px] font-black uppercase tracking-widest">FHE features require Arbitrum Sepolia</span>
          <button onClick={() => handleSwitch(PRIMARY_CHAIN.id)} disabled={switching}
            className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-white/20 hover:bg-white/30
              border border-white/30 text-[10px] font-black uppercase tracking-wider transition-all disabled:opacity-60 disabled:cursor-wait">
            {switching ? <><svg className="w-3 h-3 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>Switching…</> : "Switch Now"}
          </button>
        ...
        <div className="flex items-center gap-3">
            ...
            {isLoggedIn && isConnected ? (
              <button onClick={() => setShowWalletMenu(!showWalletMenu)}
                ...
                <div className="w-6 h-6 rounded-lg bg-indigo-600 flex items-center justify-center flex-shrink-0">
                  <span className="text-[11px] font-black text-white">{cpUser?.name?.charAt(0).toUpperCase() ?? "?"}</span>
                </div>
                <span className="text-xs font-bold text-slate-700 dark:text-slate-200 hidden sm:inline max-w-[80px] truncate">
                  {cpUser?.name?.split(" ")[0]}
                </span>
                {roleMeta && (
                  <span className={`text-[9px] font-black px-1.5 py-0.5 rounded-md border hidden lg:inline ${roleMeta.bg} ${roleMeta.text} ${roleMeta.border}`}>
                    {roleMeta.label}
                  </span>
                )}
              ...
              /* Not logged in — always show Login button regardless of wallet state */
              <LoginButton />
            )}
            ...
            {showWalletMenu && isLoggedIn && (
              <>
                ...
                <div className="absolute right-0 mt-3 w-72 rounded-2xl border border-slate-200 dark:border-slate-800
                  ...
                  <div className="px-3 py-3 mb-1">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-600 to-violet-600
                        ...
                        <span className="text-sm font-black text-white">{cpUser?.name?.charAt(0).toUpperCase() ?? "?"}</span>
                      </div>
                      ...
                      {roleMeta && (
                        <span className={`text-[9px] font-black px-2 py-1 rounded-lg border flex-shrink-0 ${roleMeta.bg} ${roleMeta.text} ${roleMeta.border}`}>
                          {roleMeta.label}
                        </span>
                      )}
                    </div>
                  ...
                  {/* Wallet */}
                  <div className="px-3 py-2 mx-1 mb-1 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/50">
                  ...
                  <div className="px-1 py-1 border-t border-slate-100 dark:border-slate-800 my-1">
                    ...
                    {switchError && <p className="text-[10px] text-red-400 font-bold px-3 pt-2">{switchError}</p>}
                  </div>
                  ...
                  <div className="border-t border-slate-100 dark:border-slate-800 pt-1">
                    <button onClick={handleCopy}
                      ...
                      {copied
                        ? <><svg className="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M5 13l4 4L19 7"/></svg><span className="text-emerald-500">Copied!</span></>
          ...
          <button className="md:hidden p-2 text-slate-600 dark:text-slate-400"
            ...
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeWidth="2.5" d={showMobileMenu ? "M6 18L18 6M6 6l12 12" : "M4 6h16M4 12h16m-7 6h7"}/>
            </svg>
          </button>
        </div>
      </div>
      ...
      {showMobileMenu && (
        <div className="md:hidden border-t border-slate-200 dark:border-slate-800
          ...
          {NAV_LINKS.map(link => (
            <a key={link.path} href={link.path} onClick={() => setShowMobileMenu(false)}
          ...
          {isLoggedIn && cpUser && (
            <div className="pt-3 border-t border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-3 px-4 py-2.5 mb-2">
                ...
                {roleMeta && (
                  <span className={`ml-auto text-[9px] font-black px-2 py-1 rounded-lg border ${roleMeta.bg} ${roleMeta.text} ${roleMeta.border}`}>
                    {roleMeta.label}
                  </span>
                )}
              </div>
